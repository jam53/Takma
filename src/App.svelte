<script lang="ts">
    ...
    async function versionCheck() {
        try {
            ...
            const response = await fetch(`https://cdn.jam54.com/Artifacts/applicationsVersions.properties?TakmaVersionCheck&v=${currentVersion}&os=${"browser"}`, { cache: "no-store" });
            const latestVersion = ((await response.text()).match(/^appVersion10\s*=\s*(.+)$/m))![1].trim();
        } catch (ex) {
            console.warn(`Failed to perform version check: ${ex}`);
        }
