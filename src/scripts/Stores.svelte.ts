...
export const isSaveLocationSet: { value: boolean } = $state({value: !(localStorage.getItem("saveDirectoryPath") === null)}); // `true` if the user has selected a save location; `false` otherwise. This is also set to `false` when the user decides to change the existing save location. Doing so re-triggers the "choose save location" screen, even if a save file already exists at the old (soon-to-be-replaced) location.
