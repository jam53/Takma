...
{#if dueDate !== null}
    <button class="dueDate"
            ...
            onclick={e => mount(DueDatePopup, {props: {clickEvent: e, dueDate, setDueDate: newDueDate => dueDate = newDueDate, setDueDateIsAllDayEvent: newDueDateIsAllDayEvent => dueDateIsAllDayEvent = newDueDateIsAllDayEvent, focusOnCardDetailsFunction: focusOnCardDetailsFunction}, target: document.body, intro: true})}
    >
        ...
        <span>
                {dueDateTitle ?? (new Date(parseInt(dueDate))).toLocaleString(SaveLoadManager.getData().displayLanguage, dueDateIsAllDayEvent ? {dateStyle: "full"} : {dateStyle: "full", timeStyle: "short", hourCycle: 'h23'})}
        </span>
    </button>
