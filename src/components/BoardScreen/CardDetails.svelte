        ...
        <div transition:slide|global class="popup {isCardFullscreen ? 'popupCardFullscreen': ''}" onclick={(e) => e.stopPropagation()}>
            ...
            <div class="bottomPart" class:bottomPartOnMobile={SaveLoadManager.getData().isUserOnMobile()}>
                <div class="cardMainAreaHolder">
                    ...
                    <DueDateChip bind:dueDate={card.dueDate} bind:dueDateIsAllDayEvent={card.dueDateIsAllDayEvent} bind:complete={card.complete} {focusOnCardDetailsFunction}/>
                    {#if showPlainTextEditor}
                ...
                <div class="cardActionsHolder">
                    ...
                    <button title={I18n.t("dueDate")}
                            onclick={e => mount(DueDatePopup, {props: {clickEvent: e, dueDate: card.dueDate, setDueDate: newDueDate => card.dueDate = newDueDate, setDueDateIsAllDayEvent: newDueDateIsAllDayEvent => card.dueDateIsAllDayEvent = newDueDateIsAllDayEvent, focusOnCardDetailsFunction: focusOnCardDetailsFunction}, target: document.body, intro: true})}
                    >
