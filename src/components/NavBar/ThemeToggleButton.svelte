<script lang="ts">
    ...
    let darkTheme = $state(saveLocationSet ? SaveLoadManager.getData().darkTheme : true); //We create an extra variable, rather than using `SaveLoadManager.getData().darkTheme`. Because otherwise the UI of the toggle/button wouldn't update, because Svelte wouldn't know that the state changed
